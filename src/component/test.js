...
const [index,sindex]=useState(0)
    ...
    return(
        <>
         ...
         <Flex ><Button position={'fixed'} left='0'id="pre" onClick={()=>{ if(index<g.length-1)sindex(index+1)}} >Pre report</Button><Spacer></Spacer><Button id="next" onClick={()=> {if(index>0)sindex(index-1)}} position={'fixed'} right='0'>Pre report</Button></Flex> 
  <Box>
    <Center>
